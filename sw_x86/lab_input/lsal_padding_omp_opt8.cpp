	...
	int *max_index, int *similarity_matrix, short *direction_matrix, int N, int M)
{
	...
	#pragma omp parallel shared(similarity_matrix, direction_matrix, max_index, max_value, diags, diag_elem, max, min, diff, string1, string2) private(col, row)
	{
	for (i=1; i<=diags; i++) {
	...
	#pragma omp parallel for schedule(static) num_threads(4)
		for (j=1; j <= diag_elem; j++) {
			...
			if (i<=min-1) {
				...
				northwest = similarity_matrix[index - 2 * min - 1];
				west = similarity_matrix[index - min - 1];
				north = similarity_matrix[index - min];
...
for (int i=0; i<N+M-1; i++) {
	for (int j=0; j<MIN(N, M); j++) {
		printf("%3d ", similarity_matrix[j+i*min]);
	}
	printf("\n");
}
...
int main(int argc, char** argv) {
	...
	int *similarity_matrix = (int*) malloc(sizeof(int) * (M+N+1) * MIN(N+1, M+1));
...
/* Create the two input strings by calling a random number generator */
	...
	memset(similarity_matrix, 0, sizeof(int) * (N+M+1) * MIN(N+1, M+1));
	...
	printf("%lf\n", t2-t1);
  //  printf(" max index is in position (%d, %d) \n", max_index[0]/MIN(N+1, M+1) , max_index[0]%MIN(N+1, M+1) );
