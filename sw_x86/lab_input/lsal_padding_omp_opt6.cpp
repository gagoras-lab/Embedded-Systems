	...
	int *max_index, int *similarity_matrix, short *direction_matrix, int N, int M)
{
	...
	int diff = M - N;
	int col= 0, row = 0;
	#pragma omp parallel shared(similarity_matrix, direction_matrix, diags, string1, string2, min, max, diff) private(diag_elem, col, row) 
	{
		for (i=1; i<= diags; i++) {
		...
		#pragma omp parallel for schedule(auto) num_threads(4) reduction(max:max_struct)
			for (j = 1; j <= diag_elem; j++) {
				...
				int dir = 0;
				int adj_nodes[3];
				// 0 northwest, 1 west, 2 north

				int index = col - 1  +  (row - 1) * N;
				adj_nodes[0] = similarity_matrix[index];
				adj_nodes[1] = similarity_matrix[index + N];
				adj_nodes[2] = similarity_matrix[index + 1];

				match = (string1[col-1]==string2[row-1]) ? MATCH : MISS_MATCH;
				val = ( adj_nodes[2] >= adj_nodes[1]) ? adj_nodes[2] + GAP_d : adj_nodes[1] + GAP_i;
				test_val = adj_nodes[0] + match;

				if (test_val >= val) {
					val = test_val;
					dir = NORTH_WEST;
				}
				else if (val == adj_nodes[2] - 1) {
					dir = NORTH;
				}
				else {
					dir = WEST;
